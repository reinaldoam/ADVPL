...
±±³Programa  ³RAGRO010 ³ Autor ³ Reinaldo Magalhães     ³ Data ³ 06.08.18 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ ORDEM DE SERVICO - ASSITENCIA TECNICA impres não fiscal TH ³±±
				    ...
				    If SB1->(DbSeek(xFilial()+AB7->AB7_CODPRO))
					   If AA3->(DbSeek(xFilial()+AB7->AB7_CODCLI+AB7->AB7_LOJA+AB7->AB7_CODPRO+AB7->AB7_NUMSER))
		 			      If AAG->(DbSeek(xFilial()+AB7->AB7_CODPRB)) //- Ocorrencias
							 ...
							 oPrint:Say(li, 0400, AllTrim(AB7->AB7_NUMSER), oFont08n,,,,0);li+=50
...
Static Function Cabec2(G)
  oPrint:Say(li, 0010, "SERVICO(S)/PECA(S)", oFont08n,,,,0)
  oPrint:Say(li, 0250, "REF.FABR.", oFont08n,,,,0)
  oPrint:Say(li, 0400, "QUANT.", oFont08n,,,,0)
  oPrint:Say(li, 0500, "LOCALIZ.", oFont08n,,,,0)
  oPrint:Say(li, 0600, "SEPARA?", oFont08n,,,,0); li += 50  
  ...
  For I:= 1 to Len(vServico)
	 ...
	 oPrint:Say(li, 0400, Transform(VServico[I][2],"@R 99,999.99"), oFont08n,,,,0)
	 oPrint:Say(li, 0500, Alltrim(VServico[I][3]), oFont08n,,,,0)
	 
	 If VServico[I][5] = "1"        
	    oPrint:Say(li, 0600, "Sim(X)Nao( ) ", oFont08n,,,,0); li += 30
	 Else
	    oPrint:Say(li, 0600, "Sim( )Nao(X) ", oFont08n,,,,0); li += 30
	 Endif   
  Next
