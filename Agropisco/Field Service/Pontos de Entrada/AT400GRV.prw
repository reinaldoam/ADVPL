...
//SA1->(DbSetOrder(1))
U_MsSetOrder("SA1","A1_FILIAL+A1_COD+A1_LOJA")//IncluМdo por Ulisses Jr em 24/04/08 em substituiГЦo a linha acima                   
SA1->(DbSeek(xFilial()+AB3->AB3_CODCLI))

//AB4->(DbSetOrder(1))
U_MsSetOrder("AB4","AB4_FILIAL+AB4_NUMORC+AB4_ITEM")//IncluМdo por Ulisses Jr em 24/04/08 em substituiГЦo a linha acima                   
...
If Alltrim(cNome) == "TECA400" .And. !Empty(AB4->AB4_NUMOS)
    //AB6->(DbSetOrder(1))
	U_MsSetOrder("AB6","AB6_FILIAL+AB6_NUMOS")//IncluМdo por Ulisses Jr em 24/04/08 em substituiГЦo a linha acima                       
...
//зддддддддддддддддддддддддддддддддддддддддддд©
//Ё Grava valor do mЦo-de-obra no orГamento  Ё
//юддддддддддддддддддддддддддддддддддддддддддды 
