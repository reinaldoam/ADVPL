...
Private cString    := "SL1"
Private nTotal
Private aEntrada:={},aSaida:={}, aOutrNf:={}, aCancAut:={}
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
  Local nOrdem, aCartao, aFatura, cMatriz, nPos, nValor, nTotNotas, cQry,cRecISS,nValISS,nValTot,cBusca, cNomeCli, nTroco
  
  Local cCfPeca  := getmv("MV_CFPECA")
  Local cCfServ  := getmv("MV_CFSERV")
  Local cTesLoca := getmv("MV_TESLOCA")
...
//  SL1->(dbSetOrder(4))  // Filial + Data ECF + PDV + Operado
  ...
  Do while !SL1->(Eof()) .and. SL1->L1_FILIAL == xFilial() .and. SL1->L1_EMISSAO <= mv_par02
 ...
 // SE5->(dbSetOrder(1)) // Filial + Data + Banco
  ...
  cQry := "SELECT L1_NUM,L1_DOC,L1_CLIENTE,L1_LOJA,L1_VLRTOT  "
  cQry += "FROM "+RetSQLName("SL1")+" SL1 "                               
  cQry += "WHERE D_E_L_E_T_ <> '*' AND L1_TIPO='V' "
  cQry += "AND L1_EMISNF BETWEEN '"+Dtos(mv_par01)+"' AND '"+Dtos(mv_par02)+"' " 
  cQry += "AND L1_OPERADO<>' ' "
  cQry += "AND L1_DOC=' ' "
  ...
  Do While !TRB->(Eof())
     ...
     AADD(aCancAut,{TRB->L1_DOC, cNomeCli, nValTot, "01", "CANC.AUTOM -> "+TRB->L1_NUM })
  ...
  // 6- NFe autorizada
  // 7- Cancelamento
  cQry := "SELECT CONVERT(VARCHAR(8000),CONVERT(BINARY(8000),XML_ERP)) AS XML_ERP,NFE_ID,CNPJDEST " 
  cQry += "FROM [TSS12].dbo.SPED050 "   
  cQry += "WHERE DATE_NFE BETWEEN '"+Dtos(mv_par01)+"' AND '"+Dtos(mv_par02)+"' " 
  cQry += "AND SUBSTRING(NFE_ID,1,3)='002' "     
  cQry += "AND STATUS<>'6' "
		
  dbUseArea( .T., "TOPCONN", TcGenQry(,,CHANGEQUERY(cQry)), "TSS", .T., .F. )
		
  dbSelectArea("TSS")
  
  Do While !TSS->(Eof())            
     cNomeCli:= Padr(Posicione("SA1",3,xFilial("SA1")+CNPJDEST,"A1_NREDUZ"),20)
     cXML:= XML_ERP
     nPosIni:= At("<vPag>",cXML)
     nPosFim:= At("</vPag>",cXML)
     nValTot:= Val(Substr(cXML,nPosIni+6,nPosFim-nPosIni-6))
     AADD(aCancAut,{Substr(NFE_ID,4,9), cNomeCli, nValTot, "01", "CANC.AUTOM TSS" })
     TSS->(DbSkip()) 
  Enddo
  ...
  Do While !TRB->(EOF())
     ...
     AADD(aOutrNf,{TRB->F1_DOC, cNomeCli, TRB->F1_VALBRUT, "01", "OUTR.ENTR." }) 
...
Static Function RecFatura(nPos)
  ...
  While !SE1->(Eof()) .and. SE1->(E1_FILIAL+E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM) == xFilial("SE1")+cBusca
     ...
     If cNat $ "CRED#REDE#VISA#DINE#AMERICAN#ELOA"
...
//////////////////////////////////
Static Function ImpResumo(nTroco)
  
  Local nDinheiro,nSangria,nSaldo,nDevoluc,nCancAut
  ...
  For i:= 1 to nLen-1
     ...
     If lAbortPrint
        ...
        Exit
     Endif
     If nLin > 55+5 // Salto de Página. Neste caso o formulario tem 60 linhas...
        ...
        @nLin, 000 PSay "Cliente                Prefixo          Dinheiro          Cheque          Debito           Elo          Redecard            Visa          Boleto         Dep.C/C           Amex            Total  Nota Fisc"
        nLin++
        @nLin, 000 PSay "--------------------  ----------  --------------  --------------  --------------  --------------  --------------  --------------  --------------  --------------  --------------  --------------  ---------"
        nLin++
     Endif
  ...
  //³ Outras notas emitidas      ³
  //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
  If nLin > 55+5 // Salto de Página. Neste caso o formulario tem 55+5 linhas...
     nLin:= Cabec(Titulo,Cabec1,Cabec2,NomeProg,Tamanho,nTipo) + 1
  Endif   
  @nLin, 000 PSay PADC("OUTRAS NOTAS - DEVOLUCOES/REMESSA/RETORNO",220)
  nLin+=2                                                  
  ...
  For i:= 1 to Len(aOutrNf)
     ...
     If nLin > 55+5 // Salto de Página. Neste caso o formulario tem 55+5 linhas...
        ...
        nLin+=2                                                            
     Endif
     @nLin, 000      PSay aOutrNf[i,2]  Picture "@!"  
     @nLin, Pcol()+2 PSay aOutrNf[i,1]  Picture "@!"
     @nLin, Pcol()+2 PSay aOutrNf[i,5]  Picture "@!"
     @nLin, 178      PSay aOutrNf[i,3]  Picture "@E 999,999,999.99"
  ...
  nLin++
  @ nLin, 000 PSay "TOTAL"
  @ nLin, 178 PSay nDevoluc Picture "@E 999,999,999.99"   
  nLin++
  @nLin, 000 PSay __PrtThinLine()                    
  nLin+=2
  ...
  If nLin > 55+5 // Salto de Página. Neste caso o formulario tem 55+5 linhas...
     nLin:= Cabec(Titulo,Cabec1,Cabec2,NomeProg,Tamanho,nTipo) + 1
  Endif   
  @nLin, 000 PSay PADC("NFCE - CANCELAMENTO AUTOMATICO",220)
  nLin+=2                                                  
  ...
  For i:= 1 to Len(aCancAut)
     ...
     If nLin > 55+5 // Salto de Página. Neste caso o formulario tem 55+5 linhas...
        ...
        nLin+=2                                                            
     Endif
     @nLin, 000      PSay aCancAut[i,2]  Picture "@!"  
     @nLin, Pcol()+2 PSay aCancAut[i,1]  Picture "@!"
     @nLin, Pcol()+2 PSay aCancAut[i,5]  Picture "@!"
     @nLin, 178      PSay aCancAut[i,3]  Picture "@E 999,999,999.99"
  ...
  nLin++
  @ nLin, 000 PSay "TOTAL"
  @ nLin, 178 PSay nCancAut Picture "@E 999,999,999.99"   
  nLin++
  @nLin, 000 PSay __PrtThinLine()                    
  nLin+=2
