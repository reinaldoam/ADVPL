...
Static Function CalcPreco
  ...
  If xTipo = "N" //- Documento tipo normal
	 For j:=1 to Len(aDoc)
  ...
  ElseIf xTipo = "F" //- Conhecimento de frete
	 
	 For i:=1 to Len(aDoc)

		SD1->(dbSeek(xFilial("SD1")+aDoc[i][1]+aDoc[i][2]+aDoc[i][3]+aDoc[i][4],.t.))
		
		Do While !SD1->(EOF()) .and. SD1->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA) = aDoc[i][1]+aDoc[i][2]+aDoc[i][3]+aDoc[i][4]
		   IncProc()
...
User Function SZ3LOk
  Local nIndVet
  If aCols[n][nUsado+1]	// Verifica se está deletado
     Return .T.
  EndIf
  ...
  For nX := 1 to Len(aCols)
	 If !aCols[nX][nUsado+1]
		...
		If SB0->(dbSeek(xFilial("SB0")+aCols[nX][2]))
			...
			SB0->B0_PRV2 := aCols[nX][8] * 1.10 // Por Reinaldo em 22.08.18
            SB0->B0_PRV3 := aCols[nX][8] * 1.20 // Por Reinaldo em 22.08.18
            SB0->B0_PRV4 := aCols[nX][8] * 1.30 // Por Reinaldo em 22.08.18
			SB0->(MsUnLock())
		Else
			RecLock("SB0",.T.)
