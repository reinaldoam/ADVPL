...
Private nPVend   := aCols[1][8]//clayson 03-04-10 //aCols[n][8]
...
oGet2	:= MSGetDados():New(110,2,200,315,4,"U_SZ3LOk()","U_SZ3TudOk()",,.T.,,,,Len(aCols),,,,,oDlg2)
...
//	ALLTRIM(cCodigo)
     If lAchou
        cQry := " UPDATE SB0020 SET B0_PRV1=A.B0_PRV1 FROM SB0010 A, SB0020 B "
	    cQry += " WHERE A.D_E_L_E_T_ <> '*' AND B.D_E_L_E_T_ <> '*' "
	    cQry += " AND A.B0_COD = '"+ALLTRIM(cRefer)+"' "
	    cQry += " AND B.B0_COD = '"+ALLTRIM(cCodigo)+"' "
     Else
 	    cQry := " INSERT INTO SB0020 (B0_FILIAL, B0_COD, B0_PRV1, R_E_C_N_O_) "
	    cQry += " SELECT B0_FILIAL, A.B1_COD, B0_PRV1, (SELECT MAX(R_E_C_N_O_)+1 NREC FROM SB0020) R_E_C_N_O_  "
	    cQry += " FROM SB0010 A, SB1020 B "
	    cQry += " WHERE A.D_E_L_E_T_ <> '*' AND B.D_E_L_E_T_ <> '*' "
	    cQry += " AND A.B0_COD = '"+ALLTRIM(cRefer)+"' "
	    cQry += " AND B.B0_COD = '"+ALLTRIM(cCodigo)+"' "
	    //cQry += " AND B1_X_REFPR = '"+ALLTRIM(cRefer)+"' "
     Endif	
     TCSQLExec(cQry)
     ...
     If lAchou
        cQry := " UPDATE DA1020 SET DA1_PRCVEN=A.DA1_PRCVEN "
	    cQry += " FROM DA1010 A, DA1020 B "
	    cQry += " WHERE A.D_E_L_E_T_ <> '*' AND B.D_E_L_E_T_ <> '*' "
	    cQry += " AND A.DA1_CODPRO = '"+ALLTRIM(cRefer)+"' "					    
	    cQry += " AND B.DA1_CODPRO = '"+ALLTRIM(cCodigo)+"' "					    
     Else
 	    cQry := " INSERT INTO DA1020 (DA1_FILIAL, DA1_CODPRO, DA1_PRCVEN, DA1_CODTAB, DA1_ATIVO, "
 	    cQry += "                     DA1_TPOPER, DA1_QTDLOT, DA1_MOEDA, R_E_C_N_O_) " 
	    cQry += " SELECT DA1_FILIAL, DA1_CODPRO, DA1_PRCVEN,'001', '1','4',999999.99,'1',"
	    cQry += " (SELECT MAX(R_E_C_N_O_)+1 NREC FROM DA1020) R_E_C_N_O_  "
	    cQry += " FROM DA1010 A, SB1020 B "
        cQry += " WHERE A.D_E_L_E_T_ <> '*' AND B.D_E_L_E_T_ <> '*' "	    
	    cQry += " AND A.DA1_CODPRO = '"+ALLTRIM(cRefer)+"' "		
	    cQry += " AND B.B1_CODPRO = '"+ALLTRIM(cCodigo)+"' "		
   	    //cQry += " AND B1_X_REFPR = '"+ALLTRIM(cRefer)+"' "
     Endif	
     TCSQLExec(cQry)
  Endif  	
