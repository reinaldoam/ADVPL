...
Static Function AGPLOJP01A()
  ...
  Private nPisCof  := 0 //Valor de Pis e Cofins           4               5          6               7            9
                      //   1         2         3          5               6          7               8            14             9
  Private aCampos  := {"Z3_ITEM","Z3_COD","Z3_XCODFAB","Z3_DESCRI","Z3_VUNIT"      ,"Z3_UCOMP" ,"Z3_VATU"    ,"Z3_VCALC"   ,"Z3_PRCFIL", "Z3_MLUCRO","Z3_DOC"   ,"Z3_SERIE","Z3_FORNECE","Z3_LOJA"}
  Private aCabec   := {"Item"   ,"Codigo","Ref.Fabric","Descricao","Valor Unitario","Ult.Preco","Valor Atual","Valor Venda","Preco Filial", "Margem de Lucro(%)"}
...
Static Function CalcPreco()
  ...
  If xTipo = "N" //- Nota fiscal normal
     ...
     For j:=1 to Len(aDoc)
		...
		Do While !SD1->(EOF()) .and. SD1->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA) = aDoc[j][1]+aDoc[j][2]+aDoc[j][3]+aDoc[j][4]
...
Static Function GeraTela()
  ...
  Private nPVend   := aCols[1][7]//clayson 03-04-10 //aCols[n][8]
...
Static Function AtuPreco()
  ...
  For nX := 1 to Len(aCols)
     ...
     If !aCols[nX][nUsado+1]
        ...
        //³ Atualiza preco do SIGALOJA    ³
        //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If SB0->(dbSeek(xFilial("SB0")+aCols[nX][2]))
		   RecLock("SB0",.F.)
		   SB0->B0_PRV1 := aCols[nX][7]	// Preço Formado pela rotina
		   SB0->(MsUnLock())
		Else
		   RecLock("SB0",.T.)
		...
		If DA1->(dbSeek(xFilial("DA1")+aCols[nX][2]))
		   RecLock("DA1",.F.)
		   DA1->DA1_PRCVEN := aCols[nX][7]	// Preço Formado pela rotina
		   DA1->(MsUnLock())
		Else
...
SZ3->Z3_XCODFAB := Posicione("SB1",1,xFilial("SB1")+SD1->D1_COD,"B1_XCODFAB")
...
If aCols[n][7] < (SD1->D1_CUSTO/SD1->D1_QUANT)
