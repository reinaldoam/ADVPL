...
// Ulisses Junior em 15/09/08
// Produto
//Formulario novo
User Function AGPENTP1()
...
While !SF1->(Eof()) .And. SF1->F1_FILIAL = xFilial("SF1");
	...
	While lImprime
		...
		While nContProd <= Len(vProd) .and. nNumItens < 20//17
			@ li,003 Psay vProd[nContProd][1]                                 // Código do Produto       // li -> 33
			@ li,019 Psay vProd[nContProd][2]                                 // Descrição do Produto    // li -> 33
			@ li,082 Psay vProd[nContProd][3]                                 // Origem+Sit. Tributaria  // li -> 33
			@ li,087 Psay vProd[nContProd][4]                                 // Unidade Medida          // li -> 33
...
li += 3
//021
@ li,004 Psay If(!SF1->F1_TIPO $ "B#D",SA2->A2_COD+" - "+SA2->A2_NOME,SA1->A1_COD+" - "+SA1->A1_NOME) //li -> 13
@ li,095 Psay If(!SF1->F1_TIPO $ "B#D",SA2->A2_CGC,SA1->A1_CGC) Picture "@R 99.999.999/9999-99" // li -> 13
@ li,127 Psay SD1->D1_EMISSAO // li -> 13
li += 2

@ li,004 Psay If(!SF1->F1_TIPO $ "B#D",SA2->A2_END,SA1->A1_END) //li -> 15
...
@ li,004 Psay If(!SF1->F1_TIPO $ "B#D",SA2->A2_MUN,SA1->A1_MUN)//li -> 17
