...
±±³Programa  ³ BLTCD001 ³ Autor ³ Alidio Ribeiro        ³ Data ³ 13/10/03 ³±±
...
/*/
User Function BLTCD001(lCallMe,xNil,cPref)
...
//- Verificando impressao de boletos por outros ambientes
If SM0->M0_CODIGO = '02' .And. Alltrim(GetEnvServer()) <> 'SCHNEIDER'
   Alert('Não é possivel gerar boleto da filial pelo ambiente ' + Alltrim(GetEnvServer())) 
   Return nil
Endif
...
If nLastKey == 27
	Set Filter to
	If xNil <> Nil
		FWrite(xNil,"06 - 01 - BLTCD001 - CANCELADA IMPRESSAO."+CHR(13)+CHR(10))
	Endif
	Return
Endif
...
If nLastKey == 27
	Set Filter to
	If xNil <> Nil
		FWrite(xNil,"06 - 02 - BLTCD001 - CANCELADA IMPRESSAO."+CHR(13)+CHR(10))
	Endif
	Return
Endif
...
If lCallMe //- Chamado pela rotina de vendas
   ...
   MV_PAR07:= SE1->E1_PORTADO  //- De Portador    
   MV_PAR08:= SE1->E1_PORTADO  //- Ate Portador   
   ...
   MV_PAR13:= Ctod("01/01/80") //- De Emissao     
   MV_PAR14:= Ctod("31/12/19") //- Ate Emissao    
   MV_PAR15:= Ctod("01/01/80") //- De Vencimento  
   MV_PAR16:= Ctod("31/12/19") //- Ate Vencimento 
   MV_PAR17:= "      "         //- Do Bordero     
   MV_PAR18:= "ZZZZZZ"         //- Ate Bordero    
Endif
...
If xNil <> Nil
	If Eof() .And. Bof()
		FWrite(xNil,"06 - 03 - BLTCD001 - NAO ENCONTROU REGISTROS."+CHR(13)+CHR(10))
	Else
		FWrite(xNil,"06 - 03 - BLTCD001 - ENCONTROU REGISTROS."+CHR(13)+CHR(10))
	Endif
Endif
...
If lExec
	If xNil <> Nil
		FWrite(xNil,"06 - 04 - BLTCD001 - CONFIRMADA IMPRESSAO."+CHR(13)+CHR(10))
	Endif
	Processa({|lEnd|MontaRel(xNil,cPref)})
Else
	If xNil <> Nil
		FWrite(xNil,"06 - 04 - BLTCD001 - CANCELADA IMPRESSAO."+CHR(13)+CHR(10))
	Endif
Endif
RetIndex("SE1")
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function MontaRel(xNil,cPref)
LOCAL oPrint
LOCAL nX := 0
Local cNroDoc :=  " "
LOCAL aDadosEmp    := {	SM0->M0_NOMECOM                                                           ,; //[1]Nome da Empresa
						...
						"CNPJ: "+Subs(SM0->M0_CGC,1,2)+"."+Subs(SM0->M0_CGC,3,3)+"."+              ; //[6]CNPJ
						Subs(SM0->M0_CGC,6,3)+"/"+Subs(SM0->M0_CGC,9,4)+"-"+                       ; //[6]CNPJ
						Subs(SM0->M0_CGC,13,2)                                                    ,; //[6]CNPJ
...
LOCAL aDadosBanco
LOCAL aDatSacado
LOCAL aBolText     := {"Juros de mora de R$ "+Transform((SE1->E1_SALDO - nVlrAbat - SE1->E1_DECRESC + SE1->E1_ACRESC) / 300, "@E 99.99")+" por dia corrido" ,;
					   "Multa de 2%" ,;
					   "Protestar apos 7 dias corridos apos o vencimento" ,;
					   "Deposito em conta somente com identificação" ,;
					   "A partir de 01/03/12 não aceitaremos pagamento de boletos em nossas lojas"}

LOCAL nI           := 1
LOCAL aCB_RN_NN    := {}

If SM0->M0_CODIGO == "02" //- Schneider
   c_Cart:= "17" // SIMPLES COM REGISTRO
Else 
   c_Cart:= "17" // SIMPLES COM REGISTRO
Endif   
...
Do While !EOF()
    ...
    If cPref <> "SC" //- Impresso pelo script
	   aDadosBanco  := { SA6->A6_COD                                             ,;  // [1]Numero do Banco
                         ...
                         c_Cart }	                                                 // [6]Codigo da Carteira
    Else 
	   aDadosBanco  := { "001"                                                   ,;  // [1]Numero do Banco
	     			     "BANCO DO BRASIL"                                       ,;  // [2]Nome do Banco
	                     "1208"                                                  ,;  // [3]Agência
                         "7148-"                                                 ,;  // [4]Conta Corrente
                         "X"                                                     ,;  // [5]Dígito da conta corrente
                         c_Cart }	                                                 // [6]Codigo da Carteira
    Endif                         
	...
	aCB_RN_NN    :=Ret_cBarra(	SE1->E1_PREFIXO	,SE1->E1_NUM	,SE1->E1_PARCELA	,SE1->E1_TIPO	,;
						...
						cNroDoc		,(E1_SALDO-nVlrAbat-E1_DECRESC+E1_ACRESC)	, c_Cart	,"9"	)

	aDadosTit	:= { AllTrim(E1_NUM)+AllTrim(E1_PARCELA) ,;  // [1] Número do título
					 E1_EMISSAO                          ,;  // [2] Data da emissão do título
					 dDataBase                    		 ,;  // [3] Data da emissão do boleto
					 E1_VENCTO                           ,;  // [4] Data do vencimento
					 (E1_SALDO - nVlrAbat - E1_DECRESC + E1_ACRESC) ,;  // [5] Valor do título (E1_SALDO - nVlrAbat)
	...
	If Marked("E1_OK")
		If xNil <> Nil
			FWrite(xNil,"06 - 05 - BLTCD001 - MARCADO - PARCELA: "+SE1->E1_PARCELA+CHR(13)+CHR(10))
		Endif
		Impress(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,xNil)
		nX := nX + 1
	Else
		If xNil <> Nil
			FWrite(xNil,"06 - 05 - BLTCD001 - DESMARCADO - PARCELA: "+SE1->E1_PARCELA+CHR(13)+CHR(10))
		Endif
	EndIf
	dbSkip()
	IncProc()
...
If xNil <> Nil .And. nI == 1
	FWrite(xNil,"06 - 07 - BLTCD001 - NAO ENTROU NO LOOP."+CHR(13)+CHR(10))
Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Impress(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,xNil)
LOCAL oFont8
LOCAL oFont11c
LOCAL oFont10
LOCAL oFont14
...
oPrint:Say  (nRow1+0200,1060,aDadosBanco[3]+"-4"+"/"+aDadosBanco[4]+aDadosBanco[5],oFont10)
...
cString := Alltrim(aDadosBanco[3]+"-4"+"/"+aDadosBanco[4]+aDadosBanco[5])
nCol := 1810+(374-(len(cString)*22))
...
cString := Alltrim(Substr(aDadosTit[6],1,11)+"-"+Substr(aDadosTit[6],12,12))
nCol := 1810+(374-(len(cString)*22))
...
oPrint:Say  (nRow2+1150-50,100 ,aBolText[1]                                        ,oFont10)
oPrint:Say  (nRow2+1200-50,100 ,aBolText[2]                                        ,oFont10)
oPrint:Say  (nRow2+1250-50,100 ,aBolText[3]                                        ,oFont10)
oPrint:Say  (nRow2+1300-50,100 ,aBolText[4]                                        ,oFont10)
oPrint:Say  (nRow2+1350-50,100 ,aBolText[5]                                        ,oFont10)
...
oPrint:Say  (nRow3+2440-50,100 ,aBolText[1]                                        ,oFont10)
oPrint:Say  (nRow3+2490-50,100 ,aBolText[2]                                        ,oFont10)
oPrint:Say  (nRow3+2540-50,100 ,aBolText[3]                                        ,oFont10)
oPrint:Say  (nRow3+2590-50,100 ,aBolText[4]                                        ,oFont10)
oPrint:Say  (nRow3+2640-50,100 ,aBolText[5]                                        ,oFont10)
...
MSBAR("INT25",25.2,0.9,aCB_RN_NN[1],oPrint,.F.,Nil,Nil,0.0235,1.7,Nil,Nil,"A",.F.)
...
If xNil <> Nil
	FWrite(xNil,"06 - 06 - BLTCD001 - IMPRESSO - PARCELA: "+SE1->E1_PARCELA+" - NOSSO NUMERO: "+SE1->E1_NUMBCO+CHR(13)+CHR(10))
Endif
...
If Empty(SE1->E1_NUMBCO) 
  ...
  NNUM := STRZERO(Val(SEE->EE_FAXATU),11)

  RecLock("SEE",.F.)

  SEE->EE_FAXATU := SomaStr(SEE->EE_FAXATU, '1') // Incrementa nosso numero . 

  MsUnlock()
  
Else 

  NNUM := STRZERO(Val(Subs(SE1->E1_NUMBCO,1,11)),11)
