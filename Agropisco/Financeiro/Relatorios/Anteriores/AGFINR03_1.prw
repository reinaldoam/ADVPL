  ...
  Do while !EOF()
     ...
     Do while !EOF() .And. cDocumen == E1_PREFIXO+E1_NUM+E1_CLIENTE+E1_LOJA
        ...
        aCB_RN_NN := Ret_cBarra( SE1->E1_PREFIXO , SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO,;
                     ...
                     aDadosBanco[7], cNroDoc , (SE1->E1_VALOR-(nVlrAbat/*+nDescFin*/)+SE1->E1_ACRESC), "18", "9")
        ...
        aDadosTit := {E1_NUM+If(Empty(E1_PARCELA),"","-"+E1_PARCELA)+;
                      ...
                      (E1_SALDO - nVlrAbat + E1_ACRESC)  ,;  // [5] Valor do título
        ...
        aBolText[4]:= "Deposito em conta somente com identificação" 
        aBolText[5]:= "A partir de 01/03/12 não aceitaremos pagamento de boletos em nossas lojas"
        aBolText[6]:= "" 
...
Endif
oPrint:Say  (nRow1+0080,180 ,"BANCO DO BRASIL" ,oFont10 )     // [2]Nome do Banco
oPrint:Say  (nRow1+0075,550,aDadosBanco[1]+"-9",oFont21 )	  // [1]Numero do Banco
...
oPrint:Say  (nRow1+0200,1060,Alltrim(substr(aDadosBanco[3],1,4)+"-"+substr(aDadosBanco[3],5,1)+"/"+aDadosBanco[4]+"-"+aDadosBanco[5]))
...
oPrint:Say  (nRow1+0250,1220,"Nosso Número",oFont8)
...
nRow1 += 20
oPrint:Say  (nRow1+0080,1750,"Cobrança Integrada BB" ,oFont14 )     // Legenda do Cabecalho
...
oPrint:Say  (nRow1+0260, 630,"(-)Outras Deduções"                          ,oFont8n)
oPrint:Say  (nRow1+0260,1070,"(+)Mora / Multa"                             ,oFont8n)

oPrint:Say  (nRow1+0260,1710,"(+)Outros Acréscimos"                        ,oFont8n)

oPrint:Say  (nRow1+0260,2050,"(=)Valor Cobrado"                            ,oFont8n)

oPrint:Say  (nRow1+0360,100 ,"Data de Emissão"                             ,oFont8n)
...
oPrint:Say  (nRow1+0540,1910,"Número.Documento"                             ,oFont8n)
oPrint:Say  (nRow1+0585,1910,aDadosTit[7]+aDadosTit[1]                      ,oFont10) //Prefixo +Numero+Parcela

oPrint:Say  (nRow1+0650,100 ,"Endereço"                                     ,oFont8n)
...
oPrint:Say  (nRow1+0750,100 ,"Município"                                    ,oFont8n)
...
oPrint:Line (nRow1+1470,100,nRow1+1470,2300 ) //nRow1+1840  - alterei para 1340

oPrint:Say  (nRow1+1480,1480,"Autenticação Mecânica"                        ,oFont8n)
oPrint:Say  (nRow1+1480,1810,"Recibo do Pagador" ,oFont10)

oPrint:Line (nRow1+1490,1280,nRow1+1490,1380 )
oPrint:Line (nRow1+1490,2200,nRow1+1490,2300 )
oPrint:Line (nRow1+1490,1280,nRow1+1550,1280 )
oPrint:Line (nRow1+1490,2300,nRow1+1550,2300 )
...
vMens[4] := "Esta quitação só terá validade após o pagamento do cheque pelo banco pagador."

oPrint:Say  (nRow1+1570,100 , vMens[1]                                     ,oFont8n)
oPrint:Say  (nRow1+1600,100 , vMens[2]                                     ,oFont8n)
oPrint:Say  (nRow1+1630,100 , vMens[3]                                     ,oFont8n)
oPrint:Say  (nRow1+1660,100 , vMens[4]                                     ,oFont8n)

/*****************/
/* SEGUNDA PARTE */
/*****************/

nRow2 := nRow1 + 725 //1025 - alterei para 525
...
oPrint:Say  (nRow3+0190,100 ,"Pagável em qualquer banco até o vencimento. Após, atualize o boleto no site bb.com.br.",oFont10)
...
oPrint:Say  (nRow3+0350,505 ,"Número.Documento"                             ,oFont8n)
oPrint:Say  (nRow3+0380,605 ,aDadosTit[7]+aDadosTit[1]                      ,oFont10) //Prefixo +Numero+Parcela

oPrint:Say  (nRow3+0350,1005,"Espécie Documento"                            ,oFont8n)
...
oPrint:Say  (nRow3+0490,100 ,"Instruções (texto de responsabilidade do beneficiário)",oFont8n)
...
oPrint:Say  (nRow3+0840,1810,"Ficha de Compensação"                        ,oFont11)
...
oPrint:Say  (nRow3+1025,1700,"Autenticação Mecânica",oFont8n)
...
Static Function Ret_cBarra(	cPrefixo,cNumero,cParcela,cTipo,cBanco,cAgencia,cConta,;
  ...
  If Empty(cNosso)   // Se ainda não foi calculado o nosso número  
  ...
  SE1->E1_NRDOC := cNosso   //- Nosso número
  MsUnlock()
