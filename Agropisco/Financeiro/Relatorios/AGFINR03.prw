...
ฑฑณPrograma  ณ AGFINR03 ณ Autor ณ Microsiga             ณ Data ณ 29/08/16 ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescri…o ณ IMPRESSAO DO BOLETO BANCO DO BRASIL COM CODIGO DE BARRAS   ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณUso       ณ Especifico para Clientes Microsiga                         ณฑฑ
ฑฑณUso       ณAlterado por Marcel Robinson Grosselli  data 22-04-10       ณฑฑ
  ...
  Do while !EOF()
     ...
     Do while !EOF() .And. cDocumen == E1_PREFIXO+E1_NUM+E1_CLIENTE+E1_LOJA
        ...
        If Empty(SA1->A1_ENDCOB) 
           aDatPagador:= {AllTrim(SA1->A1_NOME)  ,;  // [1]Razใo Social
        ...
        aDadosTit := {E1_NUM+If(Empty(E1_PARCELA),"","-"+E1_PARCELA)+;
                      ...
                      (E1_SALDO - nVlrAbat + E1_ACRESC - E1_DECRESC)  ,;  // [5] Valor do tํtulo
        ...
        aBolText[4]:= "Deposito em conta somente com identifica็ใo" 
        aBolText[5]:= "A partir de 01/03/12 nใo aceitaremos pagamento de boletos em nossas lojas"
        aBolText[6]:= "" 
...
/* PRIMEIRA PARTE */
/******************/
cString01 := Alltrim(Substr(aDadosBanco[3],1,4)+"-"+substr(aDadosBanco[3],5,1)+"/"+aDadosBanco[4]+"-"+aDadosBanco[5]) // Agencia/codigo beneficiแrio
...
nRow1 += 20
oPrint:Say  (nRow1+0080,1750,"Cobran็a Integrada BB" ,oFont14 )     // Legenda do Cabecalho
...
oPrint:Say  (nRow1+0150,100 ,"Beneficiแrio",oFont8n)
...
oPrint:Say  (nRow1+0260, 630,"(-)Outras Dedu็๕es"                          ,oFont8n)
oPrint:Say  (nRow1+0260,1070,"(+)Mora / Multa"                             ,oFont8n)

oPrint:Say  (nRow1+0260,1710,"(+)Outros Acr้scimos"                        ,oFont8n)

oPrint:Say  (nRow1+0260,2050,"(=)Valor Cobrado"                            ,oFont8n)

oPrint:Say  (nRow1+0360,100 ,"Data de Emissใo"                             ,oFont8n)
...
oPrint:Say  (nRow1+0360, 630,"Ag๊ncia / C๓digo Beneficiแrio",oFont8n)
...
oPrint:Say  (nRow1+0650,100 ,"Endere็o"                                     ,oFont8n)
...
oPrint:Say  (nRow1+0750,100 ,"Municํpio"                                    ,oFont8n)
...
oPrint:Say  (nRow1+1480-470,1480,"Autentica็ใo Mecโnica"                        ,oFont8n)
...
oPrint:Say  (nRow3+0190,100 ,"Pagแvel em qualquer banco at้ o vencimento. Ap๓s, atualize o boleto no site bb.com.br.",oFont10)
...
oPrint:Say  (nRow3+0490,100 ,"Instru็๕es (texto de responsabilidade do beneficiแrio)",oFont8n)
...
oPrint:Say  (nRow3+1025,1700,"Autentica็ใo Mecโnica",oFont8n)
...
Static Function Ret_cBarra(	cPrefixo,cNumero,cParcela,cTipo,cBanco,cAgencia,cConta,;
  ...
  Local aRet	   := {}
  Local cNumTmp    := ""
  Local lConv6Dig  := (Len(AllTrim(SEE->EE_CODEMP)) == 6)   // Conv๊nio 6 dํgitos
  Local cNosso	   := SE1->E1_NRDOC // SE1->E1_NUMBCO

  cAgencia := Left(Alltrim(cAgencia),4)

  If Empty(cNosso)   // Se ainda nใo foi calculado o nosso n๚mero  
   
     cNumTmp:= NossoNum()

     If lConv6Dig   // Conv๊nio 6 dํgitos 
