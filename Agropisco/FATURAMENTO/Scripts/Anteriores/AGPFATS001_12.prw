...
//	Cabecalho() // Impressão do Cabeçalho da Nota Fiscal.
	...
	While !SD2->(Eof()) .And. SF2->F2_FILIAL == SD2->D2_FILIAL;
		...
		If SD2->D2_TP # "MO" //Se for Produto                 
		    ...
		    nPrcProd:= If(SD2->D2_DESCON > 0, SD2->D2_PRUNIT, SD2->D2_PRCVEN)
			If nPos = 0
	...
	While lImprime
		...
		nTotal := nValorProd+nValIpi+SF2->F2_FRETE+SF2->F2_SEGURO+SF2->F2_DESPESA+nValorServ+nValIss-nDescont //IF(Alltrim(SF2->F2_ESPECIE)$"CF#NF",0,SF2->F2_DESCONT)
