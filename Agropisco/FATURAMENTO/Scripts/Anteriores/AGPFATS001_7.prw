...
//U_MsSetOrder("SF2", "F2_FILIAL+F2_SERIE+F2_DOC")//Cabeçalho da NF
SF2->(dbSetOrder(1))
...
If AllTrim(Funname()) != "LOJR130" .And. !Empty(SF2->F2_NFCUPOM)
	Mv_Par01:= Substr(SF2->F2_NFCUPOM,4,6)
	Mv_Par02:= Substr(SF2->F2_NFCUPOM,4,6)
...
//	Cabecalho() // Impressão do Cabeçalho da Nota Fiscal.
	...
	While !SD2->(Eof()) .And. SF2->F2_FILIAL == SD2->D2_FILIAL;
		...
		If SD2->D2_TP # "MO" //Se for Produto
			...
			Else
				...
				vProd[nPos][7] += (SD2->D2_PRUNIT*SD2->D2_QUANT)//SD2->D2_TOTAL
			EndIf     
			If SD2->D2_ORIGLAN == "LO" //- Vendido pelo loja
			   nValorProd := nValorProd + (SD2->D2_PRUNIT*SD2->D2_QUANT)//SD2->D2_TOTAL
			Else 
			   nValorProd := nValorProd + (SD2->D2_PRCVEN*SD2->D2_QUANT)//SD2->D2_TOTAL
			Endif   
    ...
    If !Empty(SF2->F2_NFCUPOM) 
       c_Nota := Substr(SF2->F2_NFCUPOM,4,6)
	...
	While lImprime
		...
		If nContProd > Len(vProd) .and. SF2->F2_DESCONT > 0
			@ li, 099 Psay "Desconto : "+Transform(SF2->F2_DESCONT,"@E 9,999,999.99")
		EndIf
		...
		nTotal := nValorProd+nValIpi+SF2->F2_FRETE+SF2->F2_SEGURO+SF2->F2_DESPESA+nValorServ+nValIss-SF2->F2_DESCONT//IF(Alltrim(SF2->F2_ESPECIE)$"CF#NF",0,SF2->F2_DESCONT)
...
IF SE1->(dbSeek(xFilial("SE1")+SF2->F2_SERIE+SF2->F2_DOC))
	...
	While SE1->E1_NUM == SF2->F2_DOC  
	    If !Trim(E1_TIPO) $ "IS-,IR-,IN-"
  		   AADD(Vetor1,{SE1->E1_VENCREA,SE1->(E1_VALOR-E1_ISS-E1_IRRF),SE1->E1_TIPO,SE1->E1_NUM,SE1->E1_PARCELA})
  		Endif   
		SE1->(dbSkip())
	Enddo
