...
If AllTrim(Funname()) != "LOJR130" .And. !Empty(SF2->F2_NFCUPOM)
    ...
    lTranspLj := .T.
    Mv_Par01:= Substr(SF2->F2_NFCUPOM,4,12)   //-clayson 16-04-2010
	Mv_Par02:= Substr(SF2->F2_NFCUPOM,4,12)      //-clayson 16-04-2010
	Mv_Par03:= Substr(SF2->F2_NFCUPOM,1,3)     //-clayson 16-04-2010
...
//	Cabecalho() // Impressão do Cabeçalho da Nota Fiscal.
	...
	While !SD2->(Eof()) .And. SF2->F2_FILIAL == SD2->D2_FILIAL;
		...
		If !Empty(SD2->D2_NFORI) //- Devolucao ou Beneficiamento
		   ...
		   If nPos = 0 
		      SF1->(DbSeek(xFilial("SF1")+SD2->(D2_NFORI+D2_SERIORI),.t.))
		...
		Else //Se for Serviço
			...
			SC6->(DbSeek(xFilial("SC6")+SD2->D2_PEDIDO),.t.)
    ...
    If !Empty(SF2->F2_NFCUPOM) 
       c_Nota := Substr(SF2->F2_NFCUPOM,4,12) //CLAUDIO
       c_Serie:= Substr(SF2->F2_NFCUPOM,1,3)  
       ...
       If SL1->(DbSeek(xFilial("SL1")+c_Serie+c_Nota),.t.)  
	...
	While lImprime
        ...
        */ 
		If !Empty(SF2->F2_VEND1)
			@ li,004 Psay "VEND: "+Posicione("SA3",1,xFilial("SA3")+SF2->F2_VEND1,"A3_NREDUZ")
		EndIf
		...
		If !Empty(SF2->F2_COND)
  		   @ li,004 Psay "C.PAG: "+Posicione("SE4",1,xFilial("SE4")+SF2->F2_COND,"E4_DESCRI") // "ESPACO RESERVADO PARA MENSAGENS DA NF NUM TOTAL DE 65 CARACTERES " 
  		Endif   
        ...
        If !Empty(SF2->F2_NFCUPOM) 
           c_Nota := SF2->F2_DOC //Substr(SF2->F2_NFCUPOM,4,9)
           c_SerNf:= SF2->F2_SERIE //Substr(SF2->F2_NFCUPOM,1,3)
           
           cQuery := " SELECT * "
           cQuery += " FROM "+RetSQLName("SL1")+" SL1 "   
           cQuery += " WHERE SL1.D_E_L_E_T_ <> '*' "
           cQuery += " AND L1_FILIAL  = '"+xFilial("SL1")+"' "
           cQuery += " AND L1_DOC = '"+c_Nota+"' "
           cQuery += " AND L1_SERIE = '"+c_SerNf+"' "

           dbUseArea( .T., "TOPCONN", TcGenQry(,,CHANGEQUERY(cQuery)), "WWW", .T., .F. )  
                                                             
           WWW->(dbGotop())                           

           cObsFis1 := If(Empty(WWW->L1_OBS1),"",WWW->L1_OBS1)
...
//³ Verificando a impressao do boleto  ³
...
If !Empty(SF2->F2_NFCUPOM)
   ...
   If SE1->(dbSeek(xFilial("SE1")+SF2->F2_SERIE+SF2->F2_DOC,.t.))
      ...
      If ALLTRIM(SE1->E1_TIPO) $ "BO#FI"
         ...
         //U_BLTCD001(.T.,,'SC')
         //U_AGFINR03(.T.,xNil,'SC') // AG: 1208-4
         U_AGFINR04(.T.,xNil,'SC') //AG: 5927-7
      Endif   
   Endif
Endif    
Return
...
If !Empty(SF2->F2_NFCUPOM)
	cNumNota:= Substr(SF2->F2_NFCUPOM,4,12)
Else
	cNumNota:= SF2->F2_DOC
Endif
Return cNumNota
