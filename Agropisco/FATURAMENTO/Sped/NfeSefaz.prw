...
cTipo   := iif(cTipo  == nil, PARAMIXB[1][1], cTipo)
cSerie  := iif(cSerie == nil, PARAMIXB[1][3], cSerie)
cNota   := iif(cNota  == nil, PARAMIXB[1][4], cNota)
cClieFor:= iif(cClieFor == nil, PARAMIXB[1][5], cClieFor)
cLoja   := iif(cLoja == nil, PARAMIXB[1][6], cLoja) 
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				//Merge
				aadd(aDest,FisGetEnd(SA1->A1_END)[1]) //aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[1])
				aadd(aDest,ConvType(IIF(FisGetEnd(SA1->A1_END)[2]<>0,FisGetEnd(SA1->A1_END)[2],"SN"))) //aadd(aDest,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN")))
				aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,FisGetEnd(SA1->A1_END)[4])) //				aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(SA1->A1_END,"SA1")[4]))
				...
				If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
					...
					aadd(aEntrega,MyGetEnd(SA1->A1_END,"SA1")[1]) //aadd(aEntrega,FisGetEnd(SA1->A1_END)[1])
					aadd(aEntrega,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN"))) //aadd(aEntrega,ConvType(IIF(FisGetEnd(SA1->A1_END)[2]<>0,FisGetEnd(SA1->A1_END)[2],"SN")))
					aadd(aEntrega,MyGetEnd(SA1->A1_END,"SA1")[4])//aadd(aEntrega,FisGetEnd(SA1->A1_END)[4])
			...
			Else
				...
				aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[1]) //aadd(aDest,FisGetEnd(SA2->A2_END)[1])
				aadd(aDest,ConvType(IIF(MyGetEnd(SA2->A2_END,"SA2")[2]<>0,MyGetEnd(SA2->A2_END,"SA2")[2],"SN"))) //aadd(aDest,ConvType(IIF(FisGetEnd(SA2->A2_END)[2]<>0,FisGetEnd(SA2->A2_END)[2],"SN")))
				aadd(aDest,IIF(SA2->(FieldPos("A2_COMPLEM")) > 0 .And. !Empty(SA2->A2_COMPLEM),SA2->A2_COMPLEM,MyGetEnd(SA2->A2_END,"SA2")[4]))				 //aadd(aDest,IIF(SA2->(FieldPos("A2_COMPLEM")) > 0 .And. !Empty(SA2->A2_COMPLEM),SA2->A2_COMPLEM,FisGetEnd(SA2->A2_END)[4]))				
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;
								...
								SB1->B1_UM,;
								(cAliasSD2)->D2_QUANT,;
								(cAliasSD2)->D2_TOTAL,; //IIF(!(cAliasSD2)->D2_TIPO$"IP",Iif(!(cAliasSD2)->D2_TIPO$"D",(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL),0),;
								...
								(cAliasSD2)->D2_VALFRE,;
								(cAliasSD2)->D2_SEGURO,;
								0,;//((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR),;
								(cAliasSD2)->D2_TOTAL / (cAliasSD2)->D2_QUANT,;//IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN+(((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)/(cAliasSD2)->D2_QUANT),0),;
                ...
                cPedido  := SC6->C6_PEDCLI 
		      	cCondPag := Posicione("SE4", 1, xFilial("SE4")+SF2->F2_COND, "E4_DESCRI") 
		    	cCodigo  := Alltrim(Posicione("SA7", 1, xFilial("SA7")+SF2->(F2_CLIENTE+F2_LOJA)+(cAliasSD2)->D2_COD,"A7_CODCLI"))
		    	cCodigo  := IIF(Empty(cCodigo),Alltrim(SB1->B1_COD), cCodigo)
                cUser    := Posicione("SA1", 1, xFilial("SA1")+SF2->F2_CLIENTE, "A1_COD") 
                cTes     := SC6->C6_TES 

		     	aAdd(aVetAux, {cCodigo,cPedido,cCondPag, SF2->F2_CLIENTE,cUser, cTes})
				//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
				...
				aTotal[02] += (cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT 
				...
				dbSkip()
		    EndDo
		    aTotal[02] := aTotal[02] + SF2->F2_DESPESA + SF2->F2_VALIPI +SF2->F2_FRETE
	
			// Adcionado por M·rcio Macedo p/ mostrar o numero do Orcamento do cliente.
		   If Empty(SF2->F2_PREFIXO) // Pedido de venda SC6
		      cMensCli += AllTrim("PEDIDO : "+POSICIONE("SC6",4,SF2->F2_FILIAL + SF2->F2_DOC +SF2->F2_SERIE,"C6_NUM") + "  VENDEDOR  : " + SF2->F2_VEND1) + "  COND.PAGTO: "+cCondPag
		   Else // Orcamennto SL1
		      //- Orcamento
		      cMensCli += AllTrim("ORCAMENTO : "+POSICIONE("SL1",2,SF2->F2_FILIAL + SF2->F2_SERIE + SF2->F2_DOC,"L1_NUM")+"  VENDEDOR  : " + SF2->F2_VEND1) +"  COND.PAGTO: "+cCondPag
		   Endif
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			//Adicionado por M·rcio Macedo para verificar complemento da NFE 
			If !AllTrim(SF1->F1_X_HIST) $ cMensCli
					cMensCli += AllTrim(SF1->F1_X_HIST)
			EndIf
			...
			If SF1->F1_TIPO $ "DB" 
				...
				aadd(aDest,AllTrim(SA1->A1_CGC))
				aadd(aDest,SA1->A1_NOME)
				aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[1]) //aadd(aDest,FisGetEnd(SA1->A1_END)[1])
				aadd(aDest,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN"))) //aadd(aDest,ConvType(IIF(FisGetEnd(SA1->A1_END)[2]<>0,FisGetEnd(SA1->A1_END)[2],"SN")))
				aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[4]) //aadd(aDest,FisGetEnd(SA1->A1_END)[4])
				aadd(aDest,SA1->A1_BAIRRO)
				If !Upper(SA1->A1_EST) == "EX"
					aadd(aDest,SA1->A1_COD_MUN)
					aadd(aDest,SA1->A1_MUN)				
			...
			Else
				...
				aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[1]) //aadd(aDest,FisGetEnd(SA2->A2_END)[1])
				aadd(aDest,ConvType(IIF(MyGetEnd(SA2->A2_END,"SA2")[2]<>0,MyGetEnd(SA2->A2_END,"SA2")[2],"SN"))) //aadd(aDest,ConvType(IIF(FisGetEnd(SA2->A2_END)[2]<>0,FisGetEnd(SA2->A2_END)[2],"SN")))
				aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[4]) //aadd(aDest,FisGetEnd(SA2->A2_END)[4])
				aadd(aDest,SA2->A2_BAIRRO)
				If !Upper(SA2->A2_EST) == "EX"
					aadd(aDest,SA2->A2_COD_MUN)
					aadd(aDest,SA2->A2_MUN)				
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],nX)
	Next nX
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,wInd)
...
aICMS    := iif(aICMS == nil, {}, aICMS)
aICMSST  := iif(aICMSST == nil,{},aICMSST)
aIPI     := iif(aIPI == nil, {}, aIPI)
aPIS     := iif(aPIS == nil, {}, aPIS)
aPISST   := iif(aPISST == nil, {}, aPISST)
aCOFINS  := iif(aCOFINS == nil,{}, aCOFINS)
aCOFINSST:= iif(aCOFINSST == nil, {}, aCOFINSST)
aISSQN   := iif(aISSQN == nil, {}, aISSQN)
aMed     := iif(aMed == nil, {}, aMed)
aArma    := iif(aArma == nil, {}, aArma)
aveicProd:= iif(aveicProd == nil, {}, aveicProd)
aDI		 := iif(aDI == nil, {}, aDI)
aAdi	 := iif(aAdi == nil, {}, aAdi)
aExp	 := iif(aExp == nil, {}, aExp)
...
cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],16,4)+'</vUnTrib>' //cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],16,4)+'</vUnTrib>'
...
/*If aVetAux[wInd][4] $ GetMv("MV_CODYAMA")

	If aVetAux[wInd][6] $ GetMv("MV_YTESPED")

    	cwChave := PadR(aVetAux[wInd][1],17)+SEPARATOR+aVetAux[wInd][5]+SEPARATOR+GetMv("MV_X_SUPPL")+ u_ConvPedido(aVetAux[wInd][2],10,aVetAux[wInd][6])+ iif(Alltrim(aVetAux[wInd][3]) $ GETMV("MV_YCONDPA"), Replicate(" ",23),aVetAux[wInd][3])
    	lLimpar := .F. 
 
 	Else

    	cwChave := PadR(aVetAux[wInd][1],17)+SEPARATOR+GetMv("MV_X_SUPPL")+SEPARATOR+aVetAux[wInd][5]+ u_ConvPedido(aVetAux[wInd][2],10,aVetAux[wInd][6])+ iif(Alltrim(aVetAux[wInd][3]) $ GETMV("MV_YCONDPA"), Replicate(" ",23),aVetAux[wInd][3])
    	lLimpar := .F.
...
cString += NfeTag('<infadprod>',ConvType(cwChave,,,lLimpar),lLimpar)

cString += '</det>'
Return(cString)

User Function ConvPedido(cPedido, nTam,cTes)

    Local cPed := Alltrim(cPedido)
    Local nLen := Len(cPed) 
    nTam := iif(nTam == nil, 10, nTam)

    If GetMv("MV_YTESPED") $ cTes
       cPed := Replicate("0", nTam)
//     cPed := Replicate(" ", nTam)
    elseif (nLen < nTam)
       cPed := Replicate("0", nTam - nLen) + cPed
    endif
...
aTransp := iif(aTransp == nil,{}, aTransp)
aImp    := iif(aImp == nil, {}, aImp)
aVeiculo:= iif(aVeiculo == nil, {}, aVeiculo)
aReboque:= iif(aReboque == nil, {}, aReboque)
aVol    := iif(aVol == nil, {}, aVol)
...
/*DEFAULT aPedido := {}
DEFAULT aExp	:= {}*/
aPedido := iif(aPedido == nil, {}, aPedido)
aExp	:= iif(aExp == nil, {}, aExp)
...
Static Function ConvType(xValor,nTam,nDec,lLimpar)
...
Do Case
	...
	Case ValType(xValor)=="C"
  	    lLimpar := iif(lLimpar == nil, .T., lLimpar)
		If nTam==Nil
			xValor := iif(lLimpar, AllTrim(xValor), xValor)
		EndIf
		//DEFAULT nTam := 60
		nTam := iif(nTam == nil, 60, nTam)
		
		cNovo := iif(lLimpar,AllTrim(EnCodeUtf8(NoAcento(SubStr(xValor,1,nTam)))),EnCodeUtf8(NoAcento(SubStr(xValor,1,nTam))))
EndCase
Return(cNovo)
...
Static Function NfeTag(cTag,cConteudo,lLimpar)
...
If (!Empty(AllTrim(cConteudo)) .And. IsAlpha(AllTrim(cConteudo))) .Or. Val(AllTrim(cConteudo))<>0
   if lLimpar  
      ...
      cRetorno := cTag+cConteudo+SubStr(cTag,1,1)+"/"+SubStr(cTag,2)
   endif	  
EndIf
Return(cRetorno)
...
Local nI	:=	1
//DEFAULT lContr  :=      .T.
 lContr  :=  iif(lContr == nil, .T., lContr)
...
If "ISENT"$Upper(cRet)
	cRet := ""
EndIf
If lContr .And. Empty(cRet) //If !(lContr) .And. !Empty(cRet)
	cRet := "ISENTO"
EndIf
If !lContr
	cRet := ""
