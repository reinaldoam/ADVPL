...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			//Pega descricao do pedido de venda-Parametro MV_NFESERV = 1 - Pedido + SX5 
			If !lNFeDesc
            ...
            //³ Tratamento criado para imprimir o campo memo C5_XMSGNFS             ³
            //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			//If lNatOper .And. lSC5 .And. nCont == 1 .And. !AllTrim(SC5->C5_XMSGNFS) $ cNatOper
			//   cMensAux := LerLinha(SC5->C5_XMSGNFS)
			//   cNatOper +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SC5->C5_XMSGNFS)),AllTrim(SC5->C5_XMSGNFS))
			//EndIf
    	    //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
            //³                               F I M                                 ³
            //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			...
			If dbSeek(xFilial("SX5")+"60"+RetFldProd(SB1->B1_COD,"B1_CODISS"))
				If !lNFeDesc  .And. nCont == 1
	    			...
	    			Else
						cNatOper += IIF(Empty(cNatOper),If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(SX5->X5_DESCRI,1,55))),AllTrim(SubStr(SX5->X5_DESCRI,1,55))),"")    			
	    			EndIf
	    	    ElseIf nCont == 1 
	    	        If cCodmun == "3300704-3156700"
	...
	If DbSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		If SF1->(FieldPos("F1_INSS"))<>0 .and. SF1->F1_INSS>0
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//Pega descricao do pedido de venda-Parametro MV_NFESERV = 1
				dbSelectArea("SC5")
				dbSetOrder(1)
				...
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³COLOCAR AQUI A MENSAGEM PENSONALIZADA CAMPO C5_XMSGNFS tipo Memo        ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ			
...
//////////////////////////////////
Static Function LerLinha(cMsgNfs)
  Local vLinha:= {} 
  Local cLinMsg := ""

  vLinha:= U_QuebraLinha(cMsgNfs)
...
///////////////////////////////////
User Function QuebraLinha(cMsgNfs)
  Local nLin, x, cMsgNfs, cLMsgNfs
  Local vRet := {}

  cMsgNfs := AllTrim(cMsgNfs)       // Retira os espacos em branco
  nLin    := MLCount(cMsgNfs,140)   // Calcula o numero de linhas

  For x:=1 To nLin
     cLMsgNfs := MemoLine(cMsgNfs,140,x) //64
     If Empty(cLMsgNfs)                  // Caso conteudo vazio
        Loop
     Endif
     AAdd( vRet , AFill(Array(1),Space(140)) )  // Adiciona uma linha no vetor

     If x <= nLin
        vRet[Len(vRet),1] := cLMsgNfs
        vRet[Len(vRet),1] += Space(140-Len(vRet[Len(vRet),1]))
     Endif
   Next
Return(vRet)