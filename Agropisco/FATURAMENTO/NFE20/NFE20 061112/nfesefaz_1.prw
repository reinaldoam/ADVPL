...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
			//≥Posiciona cliente ou fornecedor - Parametro MV_NFEMSA1=C               ≥
			//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ	
			If !SF2->F2_TIPO $ "DB" 
			    dbSelectArea("SA1")
				dbSetOrder(1)
					...
					If (cAliasSD2)->D2_TIPO $ "DBN"
						...
						If MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							If AllTrim(SF1->F1_ESPECIE)=="CF"
								aadd(aRefECF,{SD1->D1_DOC,SF1->F1_ESPECIE,SF2->F2_PDV}) //- por Reinaldo Magalhaes em 25.07.11 acrescentado ,SF2->F2_PDV 
							Endif  
							//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
...
//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				//- por Reinaldo Magalhaes em 17.8.11
				If !AllTrim(SC5->C5_OBS1) $ cMensCli
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					cMensCli += AllTrim(SC5->C5_OBS1)
				EndIf
				If !AllTrim(SC5->C5_OBS2) $ cMensCli
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					cMensCli += AllTrim(SC5->C5_OBS2)
				EndIf                               
				If !AllTrim(SC5->C5_OBS3) $ cMensCli
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					cMensCli += AllTrim(SC5->C5_OBS3)
				EndIf                               
				//- Fim por Reinaldo
				...
				//- Por Reinaldo Magalhaes em 06.11.12
				If !Empty(SC6->C6_XPEDCLI) 
				    aadd(aPedCom,{AllTrim(SC6->C6_XPEDCLI), AllTrim(SC6->C6_XITEMPC)})
				Else
				    aadd(aPedCom,{})
				Endif    
				//- Ate aki
				...
				If Empty(aPedido)
					aPedido := {"",AllTrim(SC6->C6_XPEDCLI),""}
				EndIf
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN+((nDesconto+(cAliasSD2)->D2_DESCZFR)/(cAliasSD2)->D2_QUANT),0),;
				...
				aadd(aAdi,{})
				aadd(aDi,{})
				//aadd(aPedCom,{}) // por Reinaldo em 30.10.12
            ...
            //- Por Reinaldo em 25.7.11
                      
            cCondPag:= Posicione("SE4", 1, xFilial("SE4")+SF2->F2_COND, "E4_DESCRI") 
            cPedCom := Posicione("SC6",4,SF2->F2_FILIAL + SF2->F2_DOC +SF2->F2_SERIE,"C6_XPEDCLI")
            
		    If Alltrim(SF2->F2_ESPECIE) <> "CF" // Pedido de venda SC6
		       cMensCli += AllTrim("PEDIDO : "+POSICIONE("SC6",4,SF2->F2_FILIAL + SF2->F2_DOC +SF2->F2_SERIE,"C6_NUM"))
		    Else
		       cMensCli += AllTrim("ORCAMENTO : "+POSICIONE("SL1",2,SF2->F2_FILIAL + SF2->F2_SERIE + SF2->F2_DOC,"L1_NUM"))
		    Endif
		    
		    cMensCli += " VENDEDOR  : " +Posicione("SA3",1,xFilial("SA3")+SF2->F2_VEND1,"A3_NREDUZ")
		    cMensCli += " COND.PAGTO: "+Alltrim(cCondPag)                   
		    cMensCli += If(!Empty(cPedCom)," S/PEDIDO: "+Alltrim(cPedCom),"")
			...
			#IFDEF TOP
				...
				If SF1->(FieldPos("F1_STATUS")) > 0 .And. SD1->(FieldPos("D1_TESACLA"))> 0 .And. SF1->F1_STATUS='C' 
					BeginSql Alias cAliasSD1			
						...
						D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN
						FROM %Table:SD1% SD1
						WHERE
				...
				Else
					BeginSql Alias cAliasSD1
						...
						D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_VUNIT,D1_CLASFIS,
						D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_TESACLA
						D1_TESACLA,D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN
						FROM %Table:SD1% SD1
						WHERE
						SD1.D1_FILIAL = %xFilial:SD1% AND
						SD1.D1_SERIE = %Exp:SF1->F1_SERIE% AND
				...
				If !Empty((cAliasSD1)->D1_NFORI) 
					If !(cAliasSD1)->D1_TIPO $ "DBN"
						...
						If MsSeek(xFilial("SD1")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_COD+(cAliasSD1)->D1_ITEMORI)
							...
							//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ|
							If AllTrim(SF1->F1_ESPECIE)=="CF"
								aadd(aRefECF,{SD1->D1_DOC,SF1->F1_ESPECIE,SF2->F2_PDV})
							Endif  
							//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
							//≥Outros documentos referenciados≥
							//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
