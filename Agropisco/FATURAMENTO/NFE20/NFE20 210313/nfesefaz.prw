...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
			dbSelectArea("SD2")                                                       //,D2_DESCZFC,D2_DESCZFP
			dbSetOrder(3)
			#IFDEF TOP
				lQuery  := .T.
				...
				BeginSql Alias cAliasSD2
						...
						D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PDV,D2_PICM %Exp:cField% 
						FROM %Table:SD2% SD2
						WHERE
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lGE .AND. SuperGetMV("MV_LJIMPGF",,".T.")
					aSb1 := GetArea("SB1")
					DbSelectArea("SB1")
					DbSetOrder(1) // B1_FILIAL+B1_COD
					If DbSeek(xFilial("SB1")+(cAliasSD2)->D2_COD)
						If SB1->B1_TIPO == SuperGetMV("MV_LJTPGAR",,"GE") 	
					    	(cAliasSD2)->(DbSkip())
							RestArea(aSb1)
							Loop
						EndIf				
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				If !Empty((cAliasSD2)->D2_IDENTB6) 
		         ...
		         EndIf 
		            ...
		            /* Tratamento da Clausula DÈcima do Ajuste SINIEF 19/2012 - CHAMADO TGCLM9 */
			...
			if ( SF3->(dbSeek(xFilial("SF3")+cSerie+cNota)) )	
				...
				if nValIPIDestac > 0
					If Len(cMensCli) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
						cMensFis += " "
					EndIf
					cMensCli += "Valor do IPI: R$ " + AllTrim(Transform(nValIPIDestac, "@ze 9,999,999,999,999.99")) + " "
				endif
			endif			
