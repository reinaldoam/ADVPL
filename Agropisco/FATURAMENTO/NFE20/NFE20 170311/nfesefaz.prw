...
Local cEspecie   := ""
Local cMensCli   := ""
Local cMensNF    := "" // Adcionado por M·rcio Macedo para imprimir a Msg na NFE.
Local cMensFis   := ""
Local cNFe       := ""
...
Private cCodigo  := ""
Private cUser    := ""
Private cTesPRural := ALLTRIM(GETMV("MV_X_TESPR")) // TES P/ Produtor Rural.           
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			#IFDEF TOP
				...
				If SD2->(FieldPos("D2_DESCZFC"))<>0 .AND. SD2->(FieldPos("D2_DESCZFP"))<>0
					BeginSql Alias cAliasSD2
						...
						D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_DESCICM
						FROM %Table:SD2% SD2
						WHERE
				...
				Else
					BeginSql Alias cAliasSD2
						SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
						D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_LOTECTL,D2_NUMLOTE,D2_DESCICM
						FROM %Table:SD2% SD2
						WHERE
						SD2.D2_FILIAL = %xFilial:SD2% AND
						SD2.D2_SERIE = %Exp:SF2->F2_SERIE% AND
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !AllTrim(SC5->C5_MENNOTA) $ cMensNF
					cMensNF += AllTrim(SC5->C5_MENNOTA)
				EndIf
				...
				(cAliasSD2)->D2_VALFRE,;
				(cAliasSD2)->D2_SEGURO,;
								(cAliasSD2)->D2_DESCICM,;//0; //((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR),;    DESCONTO
		   ...
		   if cTes $ cTesPRural // TES P/ Produtor Rural.           
 			    aTotal[02] := aTotal[02] + SF2->F2_DESPESA + SF2->F2_VALIPI +SF2->F2_FRETE - SF2->F2_VALICM
			    cMensNF := "Desconto de ICMS concedido conf. Lei N. 215 de 11 de setembro de 1998" 			    
		   Endif
		   
		   If Empty(SF2->F2_PREFIXO) // Pedido de venda SC6
		      cMensCli += AllTrim("PEDIDO : "+POSICIONE("SC6",4,SF2->F2_FILIAL + SF2->F2_DOC +SF2->F2_SERIE,"C6_NUM") + "  VENDEDOR  : " + Alltrim(SF2->F2_VEND1) ) + "  COND.PAGTO: "+Alltrim(cCondPag)
		      cMensCli += " , " + Alltrim(cMensNF)
		   Else // Orcamennto SL1
		      //- Orcamento
		      cMensCli += AllTrim("ORCAMENTO : "+POSICIONE("SL1",2,SF2->F2_FILIAL + SF2->F2_SERIE + SF2->F2_DOC,"L1_NUM")+"  VENDEDOR  : " + Alltrim(SF2->F2_VEND1) ) +"  COND.PAGTO: "+Alltrim(cCondPag)
		      cMensCli += " , " + Alltrim(cMensNF)
		   Endif
...
If cVerAmb== "2.00" 
	Do Case
		Case (!Empty(aNfVinc).And. !(aNota[5]$"NDB"))
	  		cTPNota:= "2"
