...
±±ºPrograma  ³AGFATR20  º Autor ³ REINALDO MAGALHAES º Data ³  20/02/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ FOLHA DE PAGAMENTO DE VENDEDORES                           º±±
±±º          ³                                                            º±±
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLinII)
  ...
  Private nMETPC   := getmv("MV_XMETPC")  //- Meta de peças -> R$ 50.000
  Private nMETEQ   := getmv("MV_XMETEQ")  //- Meta de equipamentos -> R$ 100.000
  Private nMINPC   := getmv("MV_XMINPC")  //- Percentual de comissão de peças < meta -> 2%
  Private nMAXPC   := getmv("MV_XMAXPC")  //- Percentual de comissão de peças > meta -> 2,5%
  ...
  Private cComGer := getmv("MV_XCOMGER") //- Vendedores que geram comissão para o gerente da loja.  
  ...
  Do while !XXX->(EOF())
     ...
     If cAuxDoc <> XXX->D2_DOC .Or. cAuxSer <> XXX->D2_SERIE
        ...
        If !Empty(XXX->F2_VEND1)
	       ...
	       If nPos > 0 
	          aFolha[nPos][3] += aVendaCom[1][1] //- Peças
	          aFolha[nPos][4] += aVendaCom[1][2] //- Equipamentos
	          aFolha[nPos][5] += aVendaCom[1][3] //- Serviços
	          aFolha[nPos][6] += aVendaCom[1][4] //- Locação                
  ...
  //aEval( aFolha, {|x| nValFat += IIF(x[1]$"000056#000079#000092#000093#000094#000095", x[3]+x[4]+x[5], 0) }) //Vendedores que geram comissão para o Diego
  aEval( aFolha, {|x| nValFat += IIF(x[1]$cComGer, x[3]+x[4]+x[5], 0) }) //Vendedores que geram comissão para o Diego
  
  For i:= 1 to Len(aFolha)     
     //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
     //³ Comissão sobre serviços   ³
     //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
     If aFolha[i][18] == "S" //- Tipo do vendedor E=Equipamento*P=peças*L=Locação*S=Serviço
        aFolha[i][13]:= nCOMSER
        aFolha[i][12]:= (aFolha[i][3]+aFolha[i][5]) * (nCOMSER/100) //- Cálculo de comissão sobre [Total = Peças + Serviços]
     Else                   
        //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
        //³ Comissão sobre serviços para vendedores de P=peças e E=equipamentos  ³
        //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
        If aFolha[i][5] > 0 .And. aFolha[i][18]$"PE"
           If aFolha[i][1] == "000056" //- Diego Barroso
              aFolha[i][13]:= 1.00
              aFolha[i][12]:= aFolha[i][5] * (1/100) //- Cálculo de comissão sobre serviços
           Else
              aFolha[i][13]:= nCOMSER
              aFolha[i][12]:= aFolha[i][5] * (nCOMSER/100) //- Cálculo de comissão sobre serviços
           Endif   
        Endif   
        //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
        //³ Comissão sobre Peças     ³
        //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
        If aFolha[i][18] == "P" //- Vendedor de peças
           If aFolha[i][3] > nMETPC //- Meta de peças -> 50k 
              aFolha[i][9]:= nMAXPC //- Percentual de comissão sobre vendas de peças > 50K -> 2,5%
           Else                
              aFolha[i][9]:= nMINPC //- Percentual de comissão sobre vendas de peças < 50k -> 2% 
           Endif 
        Else 
           If aFolha[i][1] == "000056" //- Diego Barroso
              aFolha[i][9]:= 1.00
           ...
           Endif   
        Endif   
        aFolha[i][8]:= aFolha[i][3] * (aFolha[i][9]/100) //- Cálculo de comissão sobre vendas
        ...
        If aFolha[i][18] == "E" //- Vendedor de equipamento
           If aFolha[i][4] > nMETEQ
              aFolha[i][11]:= nMAXEQ //- Percentual de comissão sobre vendas de equipamentos > 100k    
           Else
              If aFolha[i][1] == "000056" //- Diego Barroso
                 aFolha[i][11]:= 1.00
              Else   
                 aFolha[i][11]:= nMINEQ //- Percentual de comissão sobre vendas de equipamentos < 100k
              Endif   
           Endif
        Else 
           aFolha[i][11]:= 0.5
        Endif   
        aFolha[i][10]:= aFolha[i][4] * (aFolha[i][11]/100) //- Cálculo de comissão sobre vendas        
        //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
        //³ Comissão sobre locacao   ³
        //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
        If aFolha[i][6] > 0 //- Locação  
           aFolha[i][15]:= nCOMLOC
           aFolha[i][14]:= aFolha[i][6] * (nCOMLOC/100) //- Cálculo de comissão sobre vendas          
           //- Tratamento diferenciado na comissão do Diego referente a locação
           //If aFolha[i][1] $ "000056#002045"
           //   aFolha[i][15]:= 1
           //   aFolha[i][14]:= aFolha[i][6] * 0.01 //- Cálculo de comissão sobre vendas          
           //Endif
        Endif                           
     Endif
     ...
     Endif 
     aFolha[i][17] += aFolha[i][8]+aFolha[i][10]+aFolha[i][12]+aFolha[i][14]+aFolha[i][16] //- Total de comissão  
  Next  
  ...
  For i:= 1 to Len(aFolha)
     ...
     @nLin,039 PSay aFolha[i][3] Picture "@E 999,999.99" // Peças
     @nLin,051 PSay aFolha[i][4] Picture "@E 999,999.99" // Equipamento
     @nLin,063 PSay aFolha[i][5] Picture "@E 999,999.99" // Serviço
     @nLin,075 PSay aFolha[i][6] Picture "@E 999,999.99" // Locacao
     @nLin,087 PSay aFolha[i][7] Picture "@E 999,999.99" // Total
     //-
     @nLin,098 PSay aFolha[i][8]  Picture "@E 999,999.99" // Valor comissão peças
     @nLin,109 PSay aFolha[i][9]  Picture "@E 999.99"     // % Comissão peças
     @nLin,116 PSay aFolha[i][10] Picture "@E 999,999.99" // Valor comissão equipamentos
     @nLin,128 PSay aFolha[i][11] Picture "@E 999.99"     // % Comissão equipamentos
     @nLin,136 PSay aFolha[i][12] Picture "@E 999,999.99" // Valor comissão serviços
     @nLin,148 PSay aFolha[i][13] Picture "@E 999.99"     // % Comissão serviços
     @nLin,156 PSay aFolha[i][14] Picture "@E 999,999.99" // Valor comissão locação
     @nLin,168 PSay aFolha[i][15] Picture "@E 999.99"     // % Comissão locação
     @nLin,176 PSay aFolha[i][16] Picture "@E 999,999.99" // Valores de comissão extra          
     @nLin,188 PSay aFolha[i][17] Picture "@E 999,999.99" // Valor comissão total     
     nLin++
  ...
  If nRegis > 0
     ...
     nlin++                            
     @nLin,000 PSay "TOTAL"
     @nLin,038 PSay nVgPeca   Picture "@E 999,999.99" // Peças
     @nLin,051 PSay nVgEquipa Picture "@E 999,999.99" // Equipamento
     @nLin,063 PSay nVgServic Picture "@E 999,999.99" // Serviço
     @nLin,075 PSay nVgLocaca Picture "@E 999,999.99" // Locacao
     @nLin,087 PSay nVgTotal  Picture "@E 999,999.99" // Total
     //-
     @nLin,098 PSay nCgPeca   Picture "@E 999,999.99" // Valor comissão peças
     @nLin,116 PSay nCgEquipa Picture "@E 999,999.99" // Valor comissão equipamentos
     @nLin,136 PSay nCgServic Picture "@E 999,999.99" // Valor comissão serviços
     @nLin,156 PSay nCgLocaca Picture "@E 999,999.99" // Valor comissão locação
