...
±±ºPrograma  ³AGFATR01  º Autor ³ WERMESON  GADELHA  º Data ³  02/01/08   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ RELATORIO DE FATURAMENTO PO PECA, SERVIÇO E LOCAÇÃO        º±±
±±º          ³                                                            º±±
...
While !XXX->(EOF())
  ...
  //Márcio Macedo 06/04/2013 
  If cAuxVend <> XXX->F2_VEND1 .And. mv_par07 == 1 .And. cAuxVend <> ''
      nLin++
...
//       @nLin++,155 pSay "********  LOOP   SD1 **************"   
   ...
   //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
   If nLin > 55  
      if (mv_par07 == 1 .And. nAuxLin > 55) .or. mv_par07 == 2 // Salto de Página. Neste caso o formulario tem 55 linhas...
   ...
   If cAuxDoc <> XXX->D2_DOC .Or. cAuxSer <> XXX->D2_SERIE
      ...
      //alert(TRANSFORM(nBase,"@E 999,999.99"))
      ...
      @nLin,00 pSay iif(mv_par07==2,"Nota Fiscal: ","")+XXX->D2_SERIE+" - "+XXX->D2_DOC+SPACE(5)+ iif(mv_par07==2,"Cliente: ","")+SubStr(XXX->D2_CLIENTE+" - "+Posicione("SA1",1,xFilial("SA1")+XXX->D2_CLIENTE,"A1_NOME"),1,40)+;
                    ...
                    Space(5)+iif(mv_par07==2,"Comissão1: ","")+Transform(XXX->D2_COMIS1,"@E 999.99")+"%"+;
                    Space(5)+iif(mv_par07==2,"Valor Comissão1: ","")+Transform((XXX->D2_COMIS1 * nBase)/100,"@E 999.99")+;
                    Space(5)+iif(mv_par07==2,"Vendedor2: ","")+SubStr(Posicione("SA3",1,xFilial("SA3")+XXX->F2_VEND2,"A3_NOME"),1,20) +;
                    Space(5)+iif(mv_par07==2,"Comissão2: ","")+Transform(XXX->D2_COMIS2,"@E 999.99")+"%"+;
                    Space(5)+iif(mv_par07==2,"Valor Comissão2: ","")+Transform((XXX->D2_COMIS2 * nBase)/100,"@E 9,999.99")+;
...
If (nValBVen + nValCVe1 + nValCVe2) > 0 .And. mv_par07 == 1
   nLin++
   @ nLin, 000  PSay __PrtThinLine()
   nLin++
   @nLin,002 pSay "TOTAL VENDEDOR: " + Posicione("SA3",1,xFilial("SA3")+AllTrim(cAuxVend),"A3_NOME")         
...
Static Function MontaQry()
 ...
 If mv_par13 = 2 //- Comissao (somente produtos que geram comissao)
 ...
 cCampQry := " F2_DOC,  F2_SERIE, D2_DOC, D2_SERIE, D2_CF, D2_TES, D2_CLIENTE, F2_VEND1, D2_COMIS1, F2_VEND2, D2_COMIS2, F2_COND, "
 cCampQry += " D2_QUANT, D2_PRUNIT, D2_PRCVEN, D2_DESCON, D2_VALIPI, D2_VALICM, F2_VALPIS, F2_VALCOFI, "
 cCampQry += " F2_VALCSLL, D2_VALINS, D2_DESCON, D2_TOTAL, D2_ITEM "
 
 cQuery := StrTran(cQuery,"COUNT(*)SOMA", cCampQry)
...
aAdd(aRegs,{cPerg,"13","Finalidade     ?","","","mv_chd","N",01,0,0,"C","","mv_par13","Faturamento","","", "","","Comissao","","","","","","","","","","","","","","","","","","","",""})
...
Static Function QryVend(cNota, cSerie)
 ...
 cQuery := " SELECT F2_DOC, D2_DOC, D2_SERIE, D2_ITEM, D2_TOTAL "
 cQuery += "  FROM "+RetSQLName("SD2")+" SD2, "
 cQuery +=           RetSQLName("SB1")+" SB1, " 
 cQuery +=           RetSQLName("SF2")+" SF2 "
 cQuery += " WHERE SD2.D_E_L_E_T_ <> '*' "  
