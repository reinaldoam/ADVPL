...
Private CONTFL     := 01
Private m_pag      := 01
Private wnrel      := "AGFATR01" // Coloque aqui o nome do arquivo usado para impressao em disco
...
//			 01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789
 ...
 Cabec2  := ""
//           XXX - XXXXXXXXX  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX   999,999.99
EndIf
...
While !XXX->(EOF())
   ...
   If cAuxDoc <> XXX->D2_DOC .Or. cAuxSer <> XXX->D2_SERIE
      ...
      @nLin,00 pSay iif(mv_par07==2,"Nota Fiscal: ","")+XXX->D2_SERIE+" - "+XXX->D2_DOC+SPACE(5)+ iif(mv_par07==2,"Cliente: ","")+SubStr(XXX->D2_CLIENTE+" - "+Posicione("SA1",1,xFilial("SA1")+XXX->D2_CLIENTE,"A1_NOME"),1,40)+;
                    Space(3)+iif(mv_par07==2,"Valor Base: ","")+Transform(nBase,"@E 999,999.99")+;
...
//   SF4->(dbSetOrder(1))
   ...
   If (XXX->D2_CF <> cAuxTES .Or. cAuxDoc <> XXX->D2_DOC .Or. cAuxSer <> XXX->D2_SERIE)
      ...
      IF XXX->D2_CF = cCfPeca .Or. XXX->D2_CF $ "6102#6929#6108#5404#6404#5405#6405"
	      IF mv_par07 == 2
...
Static Function MontaQry()
 ...
 IF mv_par08 == 1 //PEÇA    
     ...
     cQuery += " OR D2_CF = '5404'" 
     cQuery += " OR D2_CF = '6404'"
     cQuery += " OR D2_CF = '5405'"
     cQuery += " OR D2_CF = '6405')"
...
STATIC FUNCTION MostraSoma()
	 ...
	 //IF cAuxTES = cCfPeca .Or. AllTrim(cAuxTes) = "6102" .Or. AllTrim(cAuxTes) = "6108"
	 IF cAuxTES = cCfPeca .Or. AllTrim(cAuxTes)$ "6102#6108#5404#6404#5405#6405"
...
/*  IF mv_par08 == 1 //PEÇA    
 ...
 IF mv_par08 == 1 //PEÇA    
   ...
   Elseif mv_par08 == 4  // Todos
     ...
     cQuery += " OR (D2_CF = '"+cCfServ+"' AND D2_TES <> '"+cTesLoca+"') "
     cQuery += " OR (D2_CF = '"+cCfServ+"' AND D2_TES = '"+cTesLoca+"')) "
     cQuery += " OR D2_CF = '6102' OR D2_CF = '6929' OR D2_CF = '6929' OR D2_CF = '6108' OR D2_CF = '6933' OR D2_CF = '5404' OR D2_CF = '6404' OR D2_CF = '5405' OR D2_CF = '6405')"
 EndIf                
