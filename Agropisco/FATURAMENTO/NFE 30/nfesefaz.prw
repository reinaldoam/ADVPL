...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
					    //³ Tratamento para dados adicionais da NFE - Por Reinaldo em 10.05.13     ³
					    //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ 
					    ...
					    //³ Tratamento para constar o item do pedido de compras - Por Reinaldo em 10.05.13     ³
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							Alltrim(cDescProd)+" -REF:"+SB1->B1_XCODFAB,; //- Por Reinaldo em 17/07/17
                ...
                cOriNf  := Posicione("SE1", 1, SF2->F2_PREFIXO+SF2->F2_DOC,"E1_ORIGEM")
                                                                          
		        If Alltrim(SF2->F2_ORIGEM) == 'VEN' 
		           cMensCli += AllTrim("PV : "+POSICIONE("SC6",4,SF2->F2_FILIAL + SF2->F2_DOC +SF2->F2_SERIE,"C6_NUM"))
		        Else
		           cMensCli += AllTrim("ORCAMENTO : "+POSICIONE("SL1",2,SF2->F2_FILIAL + SF2->F2_SERIE + SF2->F2_DOC,"L1_NUM"))
		        Endif
                cMensCli += " COD.CLIENTE : " +SF2->F2_CLIENTE+"/"+SF2->F2_LOJA
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			aadd(aNota,IIF(Empty(SF1->F1_HORA),Left(Time(),5),SF1->F1_HORA)) // Reinaldo em 01.04.15 - Não estava gravando F1_HORA
