		...
		#IFDEF TOP
			...
			BeginSql Alias cAliasSD2
				SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
					...
					D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_BASEISS,D2_PROJPMS,D2_DESCICM
				FROM %Table:SD2% SD2
				WHERE
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			Else 
				...
				//	cDiscrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)+" "
   		        //EndIf             
   		        // Reinaldo 26-12-12
				If lSC5 .And. !Empty(SC5->C5_OBS1).And. nCont == 1
					cDiscrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_OBS1)),SC5->C5_OBS1)+" "
				EndIf
				If lSC5 .And. !Empty(SC5->C5_OBS2).And. nCont == 1
					cDiscrNFSe += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_OBS2)),SC5->C5_OBS2)+" "
				EndIf
				// fim
			EndIf
			...
			EndIf
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Tratamento para Calcular o Desconto do Para Belo Horizonte			   ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ				
...
If Len( cCodTom ) <= 5 .And. (!(cCodTom$'99999').Or. (cCodTom$'99999' .And. SM0->M0_CODMUN $ '3550308|3300704'))
...
ElseIf cCodMun $ "4208203" //Itajaí
	
	lRetido:= aScan(aRetido,{|x| x[1] == "ISS"}) > 0
	Do Case
		Case aTotal[3] $ "1"
			If lRetido    
				cTributa := "12"   // ISS retido por tomador do município 
			Else
				cTributa := "11"   // ISS próprio
			EndIf			
	    Case aTotal[3] $ "2"
	    	If lRetido    
				cTributa := "13"   //ISS retido por tomador de fora do município
			Else
			  	cTributa := "15"  //ISS próprio pago para outro município
			EndIf  	
		Case aTotal[3] $ "3-4-7" 
			cTributa := "14"   //Isento/Imune
	    Case aTotal[3] $ "8"
	    	If cMVOPTSIMP == "1"
	    		If lRetido
		    	 	cTributa := "17" //Simples Nacional (imposto retido)
		    	 Else
		    	 	cTributa := "16" //Simples Nacional (pagamento na DAS)
		    	 EndIf
		    EndIf
		Case aTotal[3] $ "5-6"
			cTributa := "19"   //Estimativa Fiscal
	EndCase
Else
	cTributa := aTotal[3]
EndIf
